...
from mast3r_fusion.frame import Frame, Mode, SharedKeyframes, SharedStates
import numpy as np
import h5py
import io
import cv2
...
def h5_frame_indices(h5_filename):
    with h5py.File(h5_filename, "r") as f:
        indices = []
        for key in f.keys():
...
def h5_frame_id(frame_data, fallback):
    frame_id = frame_data.get("id", fallback)
    if hasattr(frame_id, "item"):
        return int(frame_id.item())
    return int(frame_id)


def create_h5_frame(frame_id, frame_data, device):
    uimg_np = frame_data["uimg"].astype(np.float32)
    if uimg_np.max() > 1.0:
        uimg_np = uimg_np / 255.0
    uimg = torch.from_numpy(uimg_np)
    img = (uimg.permute(2, 0, 1).to(device=device, dtype=torch.float32) * 2.0) - 1.0
    img_shape = frame_data.get("img_shape", torch.tensor([[uimg.shape[0], uimg.shape[1]]]))
    img_shape = torch.as_tensor(img_shape, device=device, dtype=torch.int)
    T_WC = torch.as_tensor(frame_data["T_WC"], device=device, dtype=torch.float32)
    return Frame(frame_id, img, img_shape, img_shape.clone(), uimg, T_WC)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Visualize PI3X H5 keyframes without applying check_h5.py's "
            "second image resize path."
        )
    )
    ...
    id_poses = {}
    id_to_h5_key = {}
    frame_keys = h5_frame_indices(args.h5)
    if len(frame_keys) == 0:
        raise ValueError(f"No frame_* entries found in {args.h5}.")

    print('Scanning poses...')
    first_frame = None
    for h5_key in tqdm.tqdm(frame_keys):
        dd = load_frame_from_h5(H5_FILE, h5_key)
        if first_frame is None:
            first_frame = dd
        frame_id = h5_frame_id(dd, h5_key)
        id_to_h5_key[frame_id] = h5_key
        id_poses[frame_id] = dd['T_WC'][-1,:]
        h,w = dd['uimg'].shape[:2]
    ...
    if "K" in first_frame:
        K = torch.as_tensor(first_frame["K"], device=device, dtype=torch.float32)
    else:
    ...
    selected_ids = []
    for i in tqdm.tqdm(sorted(id_to_h5_key.keys())):
        ...
        dd = load_frame_from_h5(H5_FILE, id_to_h5_key[i])
        ...
        frame = create_h5_frame(i, dd, device)
        frame.update_pointmap(dd['X'].to(device=device, dtype=torch.float32), (dd['C']/dd['N']).to(device=device, dtype=torch.float32))
        frame.feat = 0
        frame.pos = 0
        frame.dataset_idx = i
