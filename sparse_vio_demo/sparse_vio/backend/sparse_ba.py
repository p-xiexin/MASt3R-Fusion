...
class SparseBackend:
    ...
    def push(self, result: FrontendResult) -> BackendState:
        ...
        if self.imu is not None and not self.vi_initialized and self._vi_init_ready():
    ...
    def _init_latest_pose(self):
        ...
        if self.vi_initialized and self.imu is not None:
            predicted = self._predict_with_imu(prev, cur)
            if predicted is not None:
                T_world_body, velocity = predicted
                self.state.body_poses[cur.frame_id] = T_world_body
                self.state.velocities[cur.frame_id] = velocity
                self.biases[cur.frame_id] = self.biases.get(prev.frame_id, self.zero_bias)
                self.state.poses[cur.frame_id] = T_world_body @ self.T_body_camera
                return
        motion_prior = self._constant_velocity_camera_prior(prev.frame_id)
        ...
        if E is None:
            self._set_latest_pose_from_prediction(prev.frame_id, cur.frame_id, motion_prior)
            return
        inliers, R_cur_prev, t_cur_prev, _ = cv2.recoverPose(E, prev_pts, cur_pts, self.camera.K, mask=mask)
        if not self.vi_initialized:
            t_norm = float(np.linalg.norm(t_cur_prev))
            if t_norm > 1e-8:
                t_cur_prev = t_cur_prev * (self.cfg.vo_init_translation / t_norm)
    ...
    def _solve_pnp_camera_pose(self, cur: FrontendResult):
        ...
        if not ok:
            return None, debug
        proj, _ = cv2.projectPoints(object_points[inliers[:, 0]], rvec, tvec, self.camera.K, None)
    ...
    def _max_observation_parallax(self, obs):
        ...
        ref = pts[0]
        return float(np.max(np.linalg.norm(pts[1:] - ref, axis=1)))
    ...
    def _triangulate_track(self, obs):
        ...
        for frame_id, xy in obs:
            ray = K_inv @ np.array([xy[0], xy[1], 1.0], dtype=np.float64)
    ...
    def _try_vi_initialize(self):
        ...
        try:
            vi = VisualIMUAlignment(self.T_body_camera, wTcs, preintegrations, ignore_lever=True)
        except np.linalg.LinAlgError as exc:
            print(f"[sparse_vio] VI init skipped: {exc}")
            return
        scale = float(vi["s"])
        if scale <= 0.0:
            print(f"[sparse_vio] VI init rejected: non-positive scale={scale:.6f}")
            return
        ...
        for pid, point in list(self.state.points.items()):
            self.state.points[pid] = R_align @ (scale * point) + t_align
